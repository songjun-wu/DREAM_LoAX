# The algorithm is modified from pyDREAM.
# The repository of pyDREAM is :  https://github.com/LoLab-MSM/PyDREAM
...
def run_dream(parameters, likelihood, nchains=5, niterations=50000, start=None, restart=False, verbose=False, nverbose=10, tempering=False, mp_context=None, **kwargs):
    ...
    try:
        print("Start DREAM(LoAX) iteration ...")
        if tempering:
            ...
            if type(start) is list:
                args = zip([step_instance]*nchains, [niterations]*nchains, start, [verbose]*nchains, [nverbose]*nchains, np.arange(nchains), [kwargs['savePath']]*nchains, [kwargs['total_iterations']]*nchains, [kwargs['model_name']]*nchains,\
                           kwargs['limits'], [kwargs['weights']]*nchains, [kwargs['obs_all']]*nchains, [kwargs['save_limits_interval']]*nchains, [kwargs['adjust_limits_nsample']]*nchains, kwargs['hard_limits_large'], kwargs['hard_limits_small'], [kwargs['burn_in_period']]*nchains)
                ...
                args = list(zip([step_instance]*nchains, [niterations]*nchains, [start]*nchains, [verbose]*nchains, [nverbose]*nchains, np.arange(nchains), [kwargs['savePath']]*nchains, [kwargs['total_iterations']]*nchains, [kwargs['model_name']]*nchains,\
                                kwargs['limits'], [kwargs['weights']]*nchains, [kwargs['obs_all']]*nchains, [kwargs['save_limits_interval']]*nchains, [kwargs['adjust_limits_nsample']]*nchains, kwargs['hard_limits_large'], kwargs['hard_limits_small'], [kwargs['burn_in_period']]*nchains, [kwargs['param_DREAM_LoA']]*nchains))
...
def adapt_weights(weights, likeli, c):  # Added by Songjun Wu, 2024

    #delta_p = (np.mean(likeli) - likeli) * 1 / 5

    #weights = weights * (1 + delta_p)

    weights = (c - np.array(likeli)) / 2

    weights /= np.sum(weights)

    return weights

def adjust_weights(adjust_limits_nsample, weights, likeli_overalls, param_DREAM_LoA):  # Added by Songjun Wu, 2024
    likeli_overalls_mean = np.mean(likeli_overalls.reshape(adjust_limits_nsample, -1), axis=0)
    
    c = param_DREAM_LoA[3]

    if len(weights) == 1:
        weights = [adapt_weights(weights, likeli_overalls_mean, c)]

    else:
        likeli_each_objective = []
        likeli_each_objective_mean = []
        weights_each_objective_sum = np.array([])

        objective_lengths = np.array([0])
        for i in range(len(weights)):
            objective_lengths = np.append(objective_lengths, objective_lengths[-1]+len(weights[i]))
            weights_each_objective_sum = np.append(weights_each_objective_sum, np.sum(weights[i]))

        for i in range(len(weights)):
            tmp = likeli_overalls_mean[objective_lengths[i]:objective_lengths[i+1]]
            likeli_each_objective.append(tmp)
            likeli_each_objective_mean.append(np.mean(tmp))
        
        weights_each_objective_sum = adapt_weights(weights_each_objective_sum, likeli_each_objective_mean, c)

        for i in range(len(weights)):
            #print(weights[i], likeli_each_objective[i])
            weights[i] = adapt_weights(weights[i], likeli_each_objective[i], c)
            
            weights[i] = weights[i] / np.sum(weights[i]) * weights_each_objective_sum[i]
            #print(weights[i])
            #print('  ')
...
def adjust_limits(iteration, burn_in_period, adjust_limits_nsample, limits, hard_limits_large, hard_limits_small, obs_all, likeli_lowers, likeli_uppers, likeli_lower, likeli_upper, param_DREAM_LoA):   # Added by Songjun Wu, 2024
    #likeli_lowers = np.append(likeli_lowers, likeli_lower)[len(likeli_lower):]
    #likeli_uppers = np.append(likeli_uppers, likeli_upper)[len(likeli_upper):]
    likeli_lowers_mean = np.mean(likeli_lowers.reshape(adjust_limits_nsample, -1), axis=0)
    likeli_uppers_mean = np.mean(likeli_uppers.reshape(adjust_limits_nsample, -1), axis=0)

    
    likeli_too_bad  = param_DREAM_LoA[0]
    likeli_too_good = param_DREAM_LoA[1]
    b = param_DREAM_LoA[2]

    index_lower = likeli_lowers_mean>likeli_too_good
    index_upper = likeli_uppers_mean>likeli_too_good
    
    limits[:len(likeli_lower)][index_lower] += b * (likeli_lowers_mean[index_lower] - likeli_too_good) / (1 - likeli_too_good) * (obs_all[index_lower] - limits[:len(likeli_lower)][index_lower])
    limits[len(likeli_lower):][index_upper] -= b * (likeli_uppers_mean[index_upper] - likeli_too_good) / (1 - likeli_too_good) * (limits[len(likeli_lower):][index_upper] - obs_all[index_upper])

    index_lower = likeli_lowers_mean<likeli_too_bad
    index_upper = likeli_uppers_mean<likeli_too_bad
    limits[:len(likeli_lower)][index_lower] -= b * (likeli_too_bad - likeli_lowers_mean[index_lower]) / (likeli_too_bad) * (obs_all[index_lower] - limits[:len(likeli_lower)][index_lower])
    limits[len(likeli_lower):][index_upper] += b * (likeli_too_bad - likeli_uppers_mean[index_upper]) / (likeli_too_bad) * (limits[len(likeli_lower):][index_upper] - obs_all[index_upper])

    if iteration > int(burn_in_period//5) * 4 and iteration < burn_in_period:
        nonvalid_idx = limits[:len(likeli_lower)] < hard_limits_large[:len(likeli_lower)]
        limits[:len(likeli_lower)][nonvalid_idx] += np.abs(hard_limits_large[:len(likeli_lower)][nonvalid_idx] - limits[:len(likeli_lower)][nonvalid_idx]) * (5/burn_in_period)

        nonvalid_idx = limits[len(likeli_lower):] > hard_limits_large[len(likeli_lower):]
        limits[len(likeli_lower):][nonvalid_idx] -= np.abs(hard_limits_large[len(likeli_lower):][nonvalid_idx] - limits[len(likeli_lower):][nonvalid_idx]) * (5/burn_in_period)

        nonvalid_idx = limits[:len(likeli_lower)] > hard_limits_small[:len(likeli_lower)]
        limits[:len(likeli_lower)][nonvalid_idx] -= np.abs(hard_limits_small[:len(likeli_lower)][nonvalid_idx] - limits[:len(likeli_lower)][nonvalid_idx]) * (5/burn_in_period)

        nonvalid_idx = limits[len(likeli_lower):] < hard_limits_small[len(likeli_lower):]
        limits[len(likeli_lower):][nonvalid_idx] += np.abs(hard_limits_small[len(likeli_lower):][nonvalid_idx] - limits[len(likeli_lower):][nonvalid_idx]) * (5/burn_in_period)

    elif iteration > burn_in_period:       
        nonvalid_idx = limits[:len(likeli_lower)] < hard_limits_large[:len(likeli_lower)]
        limits[:len(likeli_lower)][nonvalid_idx] = hard_limits_large[:len(likeli_lower)][nonvalid_idx]

        nonvalid_idx = limits[len(likeli_lower):] > hard_limits_large[len(likeli_lower):]
        limits[len(likeli_lower):][nonvalid_idx] = hard_limits_large[len(likeli_lower):][nonvalid_idx]

        nonvalid_idx = limits[:len(likeli_lower)] > hard_limits_small[:len(likeli_lower)]
        limits[:len(likeli_lower)][nonvalid_idx] = hard_limits_small[:len(likeli_lower)][nonvalid_idx]
...
def _sample_dream(args):
    ...
    try: 
        ...
        weights = args[10]
        obs_all = args[11]
        save_limits_interval = args[12]
        adjust_limits_nsample = args[13]
        hard_limits_large = args[14]
        hard_limits_small = args[15]
        burn_in_period = args[16]
        param_DREAM_LoA = args[17]
        ...
        for iteration in range(iterations):
            ...
            if iteration==0:
                fpath_param = savePath + model_name +'_sampled_params_chain_' + str(chainID)+'_'+str(total_iterations)+'.bin'
                fpath_likeli = savePath + model_name +'_likeli_chain_' + str(chainID)+'_'+str(total_iterations)+'.bin'
                fpath_limits = savePath + model_name +'_limits_chain_' + str(chainID)+'_'+str(total_iterations)+'.bin'
                f_param = open(fpath_param, 'ab+')
                f_likeli = open(fpath_likeli, 'ab+')
                f_limits = open(fpath_limits, 'ab+')

                adjust_limits_flag = False
                likeli_lowers = np.array([])
                likeli_uppers = np.array([])
                likeli_overalls = np.array([])

                

            if iteration % save_limits_interval == 0:
                limits.tofile(f_limits)


            sampled_params, likeli, likeli_lower, likeli_upper = step_fxn(q0, chainID, limits, weights, obs_all, adjust_limits_flag, param_DREAM_LoA)  # sample parameter sets and run the model

            q0 = sampled_params

            adjust_limits_flag = False

            if iteration % adjust_limits_nsample == 0 and iteration >= adjust_limits_nsample:
                limits = adjust_limits(iteration, burn_in_period, adjust_limits_nsample, limits, hard_limits_large, hard_limits_small, obs_all, likeli_lowers, likeli_uppers, likeli_lower, likeli_upper, param_DREAM_LoA)
                weights = adjust_weights(adjust_limits_nsample, weights, likeli_overalls, param_DREAM_LoA)
                adjust_limits_flag = True

                likeli_lowers = np.array([])
                likeli_uppers = np.array([])
                likeli_overalls = np.array([])
            
            likeli_lowers = np.append(likeli_lowers, likeli_lower)
            likeli_uppers = np.append(likeli_uppers, likeli_upper)
            likeli_overalls = np.append(likeli_overalls, likeli_lower*likeli_upper)
                    

            sampled_params.tofile(f_param)
            likeli.tofile(f_likeli)

            if iteration%dream_instance.history_thin == 0:
                if chainID==0:

                    print('Likelihood of chain 0 at iteration ' + str(iteration) +' :    ' + str(likeli))
        ...
        if chainID==0:
            print('Calibration done for chain 0!')
            print('Saving outputs ...')
            print('The parameters  are saved at   :\n' + fpath_param)
            print('The likelihoods are saved at   :\n' + fpath_likeli)
            print('The limits      are saved at   :\n' + fpath_limits)
