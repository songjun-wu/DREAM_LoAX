...
class Dream():
    ...
    def __init__(self, model, variables=None, nseedchains=None, nCR=3, adapt_crossover=True, adapt_gamma=False,
        ...
        self.last_like = None
        self.last_likeli_lower = None
        self.last_likeli_upper = None

    
    def astep(self, q0, chainID, limits, weights, obs_all, adjust_limits_flag, param_DREAM_LoA, T=1., last_loglike=None, last_logprior=None):
        ...
        try:
            ...
            if self.last_like == None:
                self.last_like, self.last_likeli_lower, self.last_likeli_upper = self.logp(q0, chainID, limits, weights, obs_all)
            elif adjust_limits_flag:
                self.last_like, self.last_likeli_lower, self.last_likeli_upper = self.logp(q0, chainID, limits, weights, obs_all)
            #Evaluate logp(s)
            if self.multitry == 1:
                q_like, likeli_lower, likeli_upper = self.logp(np.squeeze(proposed_pts), chainID, limits, weights, obs_all) 
                q = np.squeeze(proposed_pts)

            # accept or reject proporsal based on likelihood
            if metrop_select(np.nan_to_num(q_like), np.nan_to_num(self.last_like), param_DREAM_LoA):
                q_new = q
                self.last_like = q_like
                self.last_likeli_lower = likeli_lower
                self.last_likeli_upper = likeli_upper
            else:
                q_new = q0
        ...
        return q_new, self.last_like, likeli_lower, likeli_upper
...
def metrop_select(likeli, likeli0, param_DREAM_LoA):
    #Perform Metropolis rejection/acceptance
    
    m = param_DREAM_LoA[-2]   # power that defines the curve of acceptance rate
    n = param_DREAM_LoA[-1]  # torlerance of likeli change
    likeli_diff = likeli0 - likeli
    # Compare acceptance ratio to uniform random number
    if likeli_diff < 0:  # if proporsal is better then accept
        # Accept proposed value
        flag =  True
    else:
        if np.random.uniform() < (  np.power(0.1, m*(likeli_diff)/likeli0) * (n - (likeli_diff)) / n  ):
            flag =  True
        else:
